const LIVE_TABLES = [
  ...
  'partner_application_field_values',
  'partner_application_extraction_jobs',
...
export function createControlCentre({
    ...
    async dashboard() {
      const [
        ...
        await Promise.all([
          ...
          supabase.rpc('get_getit_launch_queue', { p_limit: 500 }),
          ...
          supabase
            ...
            .limit(500),
          supabase
            .from('partner_application_field_definitions')
            .select('*')
            .order('sort_order', { ascending: true }),
          supabase
            .from('partner_application_field_values')
            .select('*')
            .order('created_at', { ascending: true })
            .limit(2000),
          supabase
            .from('partner_application_extraction_jobs')
            .select('*')
      ...
      return {
        ...
        launchQueue: throwIfError(launchQueue) || [],
        ...
        partnerApplicationFieldDefinitions: throwIfError(partnerApplicationFieldDefinitions) || [],
        partnerApplicationFieldValues: throwIfError(partnerApplicationFieldValues) || [],
        partnerApplicationExtractionJobs: throwIfError(partnerApplicationExtractionJobs) || [],
        partnerOnboardingRequirements: throwIfError(partnerOnboardingRequirements) || [],
        automationEvents: throwIfError(automationEvents) || [],
        supervisorRequests: throwIfError(supervisorRequests) || [],
      };
    },
  ...
  const actions = {
    createSupervisorRequest: (prompt, priority = 0) =>
      mutate('Operations supervisor', 'create_supervisor_request_v1', {
        p_prompt: prompt,
        p_priority: priority,
      }),
    ...
    addCustomerToWaitlist: (conversationId, sourceMessageId = null, note = null) =>
      mutate('Waitlist', 'staff_add_customer_to_waitlist_v1', {
        p_conversation_id: conversationId,
    ...
    reviewPartnerApplication: (applicationId, status, reviewNote, expectedVersion) =>
      mutate('Partner application', 'review_partner_application_v3', {
        p_application_id: applicationId,
        p_new_status: status,
        p_review_note: reviewNote || null,
        p_expected_version: expectedVersion,
      }),
    queuePartnerApplicationExtraction: (applicationId) =>
      mutate('Form extraction', 'queue_partner_application_extraction_v1', {
        p_application_id: applicationId,
      }),
    reviewPartnerApplicationField: ({ applicationId, fieldKey, valueText, valueJson = null, verificationStatus, staffNote = null, expectedVersion = 0 }) =>
      mutate('Application field', 'review_partner_application_field_v1', {
        p_application_id: applicationId,
    ...
    updatePartnerOnboardingRequirement: ({ requirementId, status, currentValue = null, staffNote = null, expectedVersion }) =>
      mutate('Onboarding checklist', 'update_partner_onboarding_requirement_v1', {
    ...
    addPartnerOnboardingRequirement: ({ applicationId, title, requirementLevel = 'optional', guidance = null }) =>
      mutate('Onboarding instruction', 'add_partner_onboarding_requirement_v1', {
        p_application_id: applicationId,
        p_title: title,
        p_requirement_level: requirementLevel,
        p_guidance: guidance || null,
      }),
    startPartnerGuidedOnboarding: ({ applicationId, expectedApplicationVersion, expectedConversationVersion }) =>
      mutate('Guided onboarding', 'start_partner_guided_onboarding_v1', {
        p_application_id: applicationId,
        p_expected_application_version: expectedApplicationVersion,
        p_expected_conversation_version: expectedConversationVersion,
      }),
  };
